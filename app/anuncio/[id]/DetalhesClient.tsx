...
export default function DetalhesClient({ id }: { id: string }) {
  ...
  const handleNextImage = () => setCurrentImageIndex(prev => prev === ad.fotos.length - 1 ? 0 : prev + 1)
  const handlePrevImage = () => setCurrentImageIndex(prev => prev === 0 ? ad.fotos.length - 1 : prev - 1)
    ...
    <div className="bg-gray-50 min-h-screen pb-10">
        ...
        <div className="fixed inset-0 z-[99999] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 transition-all">
           <div className="bg-white rounded-3xl w-full max-w-md p-6 relative shadow-2xl animate-in zoom-in-95 duration-200">
              ...
              <form onSubmit={handleReportSubmit}>
                 <textarea required value={reportMotivo} onChange={(e) => setReportMotivo(e.target.value)} rows={4} className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 mb-4 text-gray-800 resize-none font-medium text-sm" placeholder="Ex: É um golpe, produto falso, categoria errada, fotos impróprias..."></textarea>
                 <button type="submit" disabled={isReporting || !reportMotivo.trim()} className="w-full bg-red-500 hover:bg-red-600 text-white font-black py-4 rounded-xl transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2">
      ...
      {isZoomOpen && (
        <div className="fixed inset-0 bg-black z-[99990] flex flex-col">
          ...
          {ad.fotos.length > 1 && (
            <>
              <button onClick={(e) => { e.stopPropagation(); handlePrevImage(); }} className="absolute left-2 md:left-6 top-1/2 -translate-y-1/2 bg-black/60 text-white p-3 md:p-4 rounded-full z-50 hover:bg-black/80 outline-none"><ChevronLeft size={32}/></button>
              <button onClick={(e) => { e.stopPropagation(); handleNextImage(); }} className="absolute right-2 md:right-6 top-1/2 -translate-y-1/2 bg-black/60 text-white p-3 md:p-4 rounded-full z-50 hover:bg-black/80 outline-none"><ChevronRight size={32}/></button>
            </>
          )}
        </div>
      )}
